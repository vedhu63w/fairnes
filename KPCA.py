...
def transform(kpca, X):
	...
	K = kpca._centerer.transform(kpca._get_kernel(X, kpca.X_fit_))
	return np.dot(K, kpca.alphas_ / np.sqrt(kpca.lambdas_))

kpca = KernelPCA()
kpca.fit(X)
X_tranformed = transform(kpca, X)
